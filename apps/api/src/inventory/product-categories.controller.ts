...
import type { CreateProductCategoryDto, UpdateProductCategoryDto } from './dto';
...
export class ProductCategoriesController {
  ...
  async findAll(@ActiveOrganization() orgId: string | undefined) {
    ...
    const result = await listProductCategories(db, { organizationId });
  ...
  async create(
    ...
    const organizationId = this.requireActiveOrganization(orgId);
    const result = await createProductCategory(db, { ...dto, organizationId });
    if (!result.success) throw this.mapError(result.error);
    return result.data;
  }
  ...
  async update(
    ...
    const organizationId = this.requireActiveOrganization(orgId);
    const result = await updateProductCategory(db, {
      ...dto,
      id,
      organizationId,
    });
  ...
  async remove(
    ...
    const organizationId = this.requireActiveOrganization(orgId);
    const result = await deleteProductCategory(db, { id, organizationId });
    if (!result.success) throw this.mapError(result.error);
    return result.data;
  }
