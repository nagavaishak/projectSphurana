...
/**
 * Schedule an Instagram Post — Connected Org (apps/app)
 *
 * Creates a real social post targeting Instagram via the content calendar's
 * Schedule Content dialog, asserts it renders on the calendar AND exists via
 * the API, then deletes it through the post side-panel.
 *
 * NOTE ON NAMING: this spec does not *publish* to Instagram. It schedules a
 * post ~30 minutes out and deletes it well before the publisher picks it up —
 * nothing is ever pushed to the live account. `schedule-instagram-post` would
 * be the honest name.
 *
 * Preconditions are SEEDED and ASSERTED, never observed-and-skipped:
 *   - an active Instagram page on the connected org's Meta integration;
 *     missing = HARD FAIL (the whole point of the spec is the Instagram target,
 *     so falling back to "any page" would have silently tested Facebook)
 *   - one uniquely-named image asset, seeded via POST /assets — Instagram
 *     requires media, so the picker must never be empty
 *
 * Auth: connected-user — applied automatically via the `connected` project.
 */

const TITLE_PREFIX = 'E2E IG Post';
const ASSET_PREFIX = 'E2E IG Asset';
 ...
 *
 * Hard-fails when there isn't one. A missing Instagram page is a broken
 * precondition of the `connected` project, i.e. exactly the regression this
 * spec exists to catch — not an environmental constraint to skip on.
 */
  ...
  if (!match) {
    const seen =
      pages
        ...
        .join(', ') || 'none';
    throw new Error(
      `Connected org has no active ${platform} page — GET /integrations/meta-ads/pages returned: ${seen}. Reconnect the Instagram account on the connected org (setup-connected seeds the Meta side) rather than skipping the test.`
    );
  }
...
test.describe('Social Posts — Instagram', () => {
  ...
  test('schedules an Instagram post and cleans up', async ({
    page,
    request,
  }) => {
    ...
    const caption = `E2E Instagram test — will be deleted ${stamp}`;
    ...
    const instagramPage = await requireActivePage(seed, 'instagram');
    ...
    await test.step('select the Instagram page', async () => {
      ...
      const option = page
        .locator('[cmdk-item]')
        .filter({ hasText: instagramPage.label });
      await expect(option).toBeVisible({ timeout: 15_000 });
      await option.click();
      await page.keyboard.press('Escape');

      // With exactly one page selected the trigger renders that page's name.
      await expect(pagesButton).toContainText(instagramPage.label);
    });
