...
/** RGBA color tuple [red, green, blue, alpha] with values 0-255 for RGB and 0-1 for alpha */
...
export const ColorPicker = ({
  ...
  // Track whether the state change originated from syncing the controlled value
  ...
  useEffect(() => {
    if (value) {
      ...
      setHue(hsl.hue() || 0);
      setSaturation(hsl.saturationl() || 0);
      setLightness(hsl.lightness() || 0);
      setAlpha((color.alpha() ?? 1) * 100);
    }
  }, [value]);

  // Notify parent of changes (skip when syncing from controlled value)
  useEffect(() => {
...
type PercentageInputProps = Omit<ComponentProps<typeof Input>, 'value'> & {
