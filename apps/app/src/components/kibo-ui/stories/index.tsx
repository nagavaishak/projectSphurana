...
export const StoryVideo = ({ className, ...props }: StoryVideoProps) => {
  ...
  const handleLoadedMetadata = () => {
    // Safari doesn't render a preview frame with preload="metadata" alone.
    // Seeking to a small offset forces it to decode and display a frame.
    if (videoRef.current) {
      videoRef.current.currentTime = initialTimeRef.current || 0.001;
    }
  };
