...
const CSS_COLOR_RE =
  /^(#[0-9a-fA-F]{3,8}|rgba?\([\d\s,./%]+\)|hsla?\([\d\s,./%]+\)|oklch\([\d\s,./%]+\)|[a-zA-Z]+)$/;

const CSS_KEY_RE = /^[a-zA-Z0-9-]+$/;

function sanitizeCssColor(color: string): string | null {
  return CSS_COLOR_RE.test(color.trim()) ? color.trim() : null;
}

function sanitizeCssKey(key: string): string | null {
  return CSS_KEY_RE.test(key) ? key : null;
}
