...
export function UsageBanner({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={cn('border-t', compact ? 'px-4 py-1.5' : 'px-4 py-2')}>
      {renderBannerForState(state, compact)}
    </div>
  );
}

function renderBannerForState(state: UsageBannerState, compact: boolean) {
  const subtleText = compact ? 'text-[10px]' : 'text-xs';
  switch (state.kind) {
    case 'subtle':
      return (
        <div
          className={cn(
            'mx-auto flex max-w-3xl items-center justify-between gap-3 text-muted-foreground',
            subtleText
          )}
        >
          <span>
    ...
    case 'warning':
      return (
        <Alert
          className={cn(
            'mx-auto max-w-3xl border-amber-500/50 bg-amber-50 text-amber-900 dark:border-amber-500/30 dark:bg-amber-950/50 dark:text-amber-200 [&>svg]:text-amber-600 dark:[&>svg]:text-amber-400',
            compact && 'py-2 text-xs [&>svg]:size-3.5'
          )}
        >
    ...
    case 'daily-limit-paid':
      return (
        <Alert
          variant="destructive"
          className={cn('mx-auto max-w-3xl', compact && 'py-2 text-xs')}
        >
          <AlertCircle className="size-4" />
          <AlertDescription className="flex items-center justify-between gap-2">
    ...
    case 'monthly-limit-free':
      return (
        <div
          className={cn(
            'mx-auto flex max-w-3xl items-center justify-between gap-3 rounded-md border border-primary/40 bg-primary/5',
            compact ? 'px-4 py-1.5 text-xs' : 'px-4 py-2 text-sm'
          )}
        >
    ...
    case 'free-tier-active':
      return (
        <div
          className={cn(
            'mx-auto flex max-w-3xl items-center justify-between gap-3 rounded-md border bg-muted/40 text-muted-foreground',
            compact ? 'px-4 py-1.5 text-[10px]' : 'px-4 py-2 text-xs'
          )}
        >
          <span>
