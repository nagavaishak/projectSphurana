...
interface RemoveMembershipPlanLocationInput {
  planId: string;
  locationId: string;
}

interface UseRemoveMembershipPlanLocationOptions {
  onSuccess?: () => void;
}
 ...
 */
export const useRemoveMembershipPlanLocation = (
  options?: UseRemoveMembershipPlanLocationOptions
) => {
  const queryClient = useQueryClient();

  const mutation = useMutation({
    mutationFn: ({ planId, locationId }: RemoveMembershipPlanLocationInput) =>
      apiClient.delete(`membership-plans/${planId}/locations/${locationId}`),
  ...
  return {
    removeMembershipPlanLocation: mutation.mutate,
    removeMembershipPlanLocationAsync: mutation.mutateAsync,
