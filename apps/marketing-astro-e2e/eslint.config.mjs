 ...
 *
 * This is a deliberate copy of apps/app-e2e/eslint.config.mjs: the portal specs
 * moved here with the portal itself, and the gate has to move with them or the
 * standard silently stops applying. There is no suppressions file — this suite
 * starts clean, so keep it that way. If the two configs need to diverge, say
 * why here; otherwise keep them identical.
 */
export default [
  {
    files: ['src/**/*.ts'],
