...
import { notifyPractitionerCancellation } from './notify-practitioner-cancellation.service.js';

const mockSendEmail = vi.mocked(sendEmail);

describe('notifyPractitionerCancellation', () => {
  ...
  it('should send cancellation notification email', async () => {
    ...
    const result = await notifyPractitionerCancellation(
      mockDb as never,
      validInput
    );
  ...
  it('should return VALIDATION_ERROR for missing appointmentId', async () => {
    await expectResult(
      notifyPractitionerCancellation(mockDb as never, {
        ...validInput,
        appointmentId: '',
      })
