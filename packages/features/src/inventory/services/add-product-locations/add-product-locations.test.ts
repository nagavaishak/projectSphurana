...
import { addProductLocations } from './add-product-locations.service.js';

/**
 * The write behind "import from another location" for products.
 *
 * The two behaviours worth pinning are the ones that differ from the REPLACING
 * `assign…Locations`: a product stocked everywhere must not be narrowed to one
 * branch, and a re-sent import must not fail.
 */
describe('addProductLocations', () => {
  ...
  it('adds the branch, leaving the existing assignments alone', async () => {
    ...
    const result = await addProductLocations(mockDb as never, input);
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      { productId: 'prod-1', locationId: 'loc-2' },
    ]);
    // Nothing is removed — the whole difference from the PUT.
    expect(mockDb.delete).not.toHaveBeenCalled();
  });

  it('does NOT narrow a product that is stocked everywhere', async () => {
  ...
  it('returns NOT_FOUND for a record outside the org', async () => {
    mockDb.query.product.findFirst.mockResolvedValueOnce(null as never);
  ...
  it('rejects an empty branch list', async () => {
    const result = await addProductLocations(mockDb as never, {
      ...input,
      locationIds: [],
    });
