...
import { removeProductLocation } from './remove-product-location.service.js';

/**
 * Taking ONE branch off a product.
 *
 * Both hard cases are here because both fail SILENTLY if written naively:
 * a product with no rows is stocked everywhere (so there is no row to delete
 * and doing nothing leaves it in place), and emptying the table reads as
 * "everywhere" — the opposite of a withdrawal.
 */
describe('removeProductLocation', () => {
  ...
  it('removes the branch and leaves the others', async () => {
    ...
    const result = await removeProductLocation(mockDb as never, input);
  ...
  it('MATERIALISES the complement when it was stocked everywhere', async () => {
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      ...
      { productId: 'prod-1', locationId: 'loc-3' },
    ]);
