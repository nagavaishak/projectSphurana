...
import { updateProductCategory } from './update-product-category.service.js';
...
describe('updateProductCategory', () => {
  ...
  it('updates a product category', async () => {
    ...
    await expectResult(
      updateProductCategory(mockDb as never, validInput)
    ).toSucceedWith((data) => {
  ...
  it('returns VALIDATION_ERROR for empty name', async () => {
    await expectResult(
      updateProductCategory(mockDb as never, { ...validInput, name: '' })
