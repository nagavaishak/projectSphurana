...
import { assignMembershipPlanLocations } from './assign-membership-plan-locations.service.js';

describe('assignMembershipPlanLocations', () => {
  ...
  it('replaces the assignments', async () => {
    ...
    const result = await assignMembershipPlanLocations(
      mockDb as never,
      validInput
    );
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      { planId: 'plan-1', locationId: 'loc-1' },
    ]);
  });

  it('an EMPTY array clears the rows — which means "sold at everywhere"', async () => {
    ...
    const result = await assignMembershipPlanLocations(mockDb as never, {
      ...validInput,
      locationIds: [],
    });
  ...
  it('de-duplicates a repeated branch instead of hitting the unique index', async () => {
    ...
    await assignMembershipPlanLocations(mockDb as never, {
      ...validInput,
  ...
  it('refuses a branch belonging to another org, and writes nothing', async () => {
    ...
    await expectResult(
      assignMembershipPlanLocations(mockDb as never, {
        ...validInput,
  ...
  it('returns NOT_FOUND when the parent belongs to another org', async () => {
    ...
    await expectResult(
      assignMembershipPlanLocations(mockDb as never, validInput)
