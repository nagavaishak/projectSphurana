...
import { addOfferLocations } from './add-offer-locations.service.js';

/**
 * The write behind "import from another location" for promotions.
 *
 * The two behaviours worth pinning are the ones that differ from the REPLACING
 * `assign…Locations`: a promotion run everywhere must not be narrowed to one
 * branch, and a re-sent import must not fail.
 */
describe('addOfferLocations', () => {
  ...
  it('adds the branch, leaving the existing assignments alone', async () => {
    ...
    const result = await addOfferLocations(mockDb as never, input);
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      { offerId: 'offer-1', locationId: 'loc-2' },
    ]);
    // Nothing is removed — the whole difference from the PUT.
    expect(mockDb.delete).not.toHaveBeenCalled();
  });

  it('does NOT narrow a promotion that is run everywhere', async () => {
  ...
  it('returns NOT_FOUND for a record outside the org', async () => {
    mockDb.query.offer.findFirst.mockResolvedValueOnce(null as never);
  ...
  it('rejects an empty branch list', async () => {
    const result = await addOfferLocations(mockDb as never, {
      ...input,
      locationIds: [],
    });
