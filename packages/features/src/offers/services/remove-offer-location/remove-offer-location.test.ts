...
import { removeOfferLocation } from './remove-offer-location.service.js';

/**
 * Taking ONE branch off a promotion.
 *
 * Both hard cases are here because both fail SILENTLY if written naively:
 * a promotion with no rows is run everywhere (so there is no row to delete
 * and doing nothing leaves it in place), and emptying the table reads as
 * "everywhere" — the opposite of a withdrawal.
 */
describe('removeOfferLocation', () => {
  ...
  it('removes the branch and leaves the others', async () => {
    ...
    const result = await removeOfferLocation(mockDb as never, input);
  ...
  it('MATERIALISES the complement when it was run everywhere', async () => {
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      ...
      { offerId: 'offer-1', locationId: 'loc-3' },
    ]);
