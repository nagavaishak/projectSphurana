...
import { removeServiceLocation } from './remove-service-location.service.js';

/**
 * Taking ONE branch off a service.
 *
 * Both hard cases are here because both fail SILENTLY if written naively:
 * a service with no rows is offered everywhere (so there is no row to delete
 * and doing nothing leaves it in place), and emptying the table reads as
 * "everywhere" — the opposite of a withdrawal.
 */
describe('removeServiceLocation', () => {
  ...
  it('removes the branch and leaves the others', async () => {
    ...
    const result = await removeServiceLocation(mockDb as never, input);
  ...
  it('MATERIALISES the complement when it was offered everywhere', async () => {
    ...
    expect(mockDb.values).toHaveBeenCalledWith([
      { serviceId: 'svc-1', locationId: 'loc-2' },
      { serviceId: 'svc-1', locationId: 'loc-3' },
    ]);
